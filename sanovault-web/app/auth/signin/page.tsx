...
const FEATURES = [
  ...
  },
  {
    title: "AI Insights",
    description:
      "Get clear summaries and trends from your lab results and records.",
    icon: (
      <svg
        className="h-5 w-5"
        fill="none"
  ...
  },
  {
    ...
    description:
      "Manage health records for everyone in your family, in one place.",
    icon: (
      <svg
        className="h-5 w-5"
        fill="none"
...
function SignInContent() {
  ...
  useEffect(() => {
    ...
    if (oauthError === "magic_link") {
      setError(
        "That sign-in link did not work. Request a new one, or use Google.",
      );
    } else if (oauthError?.startsWith("oauth_")) {
      setError(
        "Social sign-in failed. Ensure this provider is enabled in Neon Auth and try again.",
      );
    }
  ...
  const inputClassName =
    "mt-1.5 block w-full rounded-lg border border-silver px-3.5 py-2.5 text-ink placeholder-blue-slate shadow-none focus:border-sage focus:outline-none focus:ring-2 focus:ring-sage/30 sm:text-sm";

  return (
    <div className="min-h-screen lg:grid lg:grid-cols-2">
      <section className="relative hidden flex-col justify-center bg-gradient-to-br from-sage/25 via-cream to-white px-6 py-12 sm:px-10 lg:flex lg:px-14 lg:py-16">
        <div className="mx-auto w-full max-w-md lg:mx-0 lg:max-w-lg">
          ...
          <p className="mt-3 text-base text-slate sm:text-lg">
            Store records securely, understand what they mean, and keep
            everyone&apos;s care in sync.
          </p>

          <ul className="mt-10 space-y-5">
            {FEATURES.map((feature) => (
              <li key={feature.title} className="flex gap-3.5">
                <span className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-sage/25 text-ink">
                  {feature.icon}
                </span>
                <div>
        ...
        <div className="mx-auto w-full max-w-md">
          ...
          <div>
            ...
            <p className="mt-1.5 text-sm text-slate">
              Use Google, or a link emailed to you. You can also use a password
              if you have one.
            </p>
          </div>
          ...
          <div className="mt-8 space-y-6">
            {message && (
              <div
                role="status"
                className="rounded-lg border border-sage/50 bg-sage/15 px-4 py-3"
              >
            ...
            <a
              ...
              className="inline-flex min-h-11 w-full items-center justify-center text-sm font-medium text-ink underline decoration-sage decoration-2 underline-offset-4 hover:text-slate"
            >
              ...
              >
                ...
                <div>
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <a
                      href={`/auth/forgot-password?${new URLSearchParams({ callbackUrl, email: email.trim().toLowerCase() })}`}
            ...
            <p className="text-center text-sm text-gray-600">
              ...
              <a
                ...
                className="font-medium text-ink underline decoration-sage underline-offset-4 hover:text-slate transition-colors"
              >
                Sign Up
              </a>
            </p>
