...
import { getPublicDocumentShare, isShareTokenShape } from '@/lib/services/document-share.service';
...
export async function GET(_: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  try {
    ...
    if (!token || !isShareTokenShape(token)) throw new AppError('Share link not found', 404);
